...
public class SvcInvoiceImp implements SvcInvoice {
	...
	private DtoCustomer getCustomer(Integer customerCode) {
		try {
			...
			throw new ApiException(HttpStatus.NOT_FOUND, "Invalid customer code: " + customerCode);
		}
	}
	...
	private DtoProduct getProduct(String productCode) {
		try {
			...
			throw new ApiException(HttpStatus.NOT_FOUND, "Invalid product code: " + productCode);
		}
	}
	...
	private void setCostsInvoice(Invoice invoice) {
		if(invoice.getStatus() != 1)
			throw new ApiException(HttpStatus.BAD_REQUEST, "Cannot retrieve canceled invoice");
